...
fn main() {
    ...
    let mut grid = vec![vec![0u32; (max_x - min_x) as usize + 1]; (max_y - min_y) as usize + 1];
    let mut steps = 0;
    for s in pipe1.iter() {
        for p in p1.points_along(s) {
            steps += 1;
            grid[p.y as usize][p.x as usize] = steps;
        }
        p1 = p1.shift(s);
    }
    ...
    for s in pipe2.iter() {
        for p in p2.points_along(s) {
            steps += 1;
            let c = grid[p.y as usize][p.x as usize];
            if c > 0 {
                crossings.push(c + steps)
            }
        }
        p2 = p2.shift(s);
    }

    println!("{}", crossings.iter().min().unwrap());
